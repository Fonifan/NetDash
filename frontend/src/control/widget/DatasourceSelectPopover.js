                ...
                <PopoverBody>
                    <form onSubmit={onSelect}>
                        <FormControl id='datasource' className={classes.control}>
                            ...
                            <Select onChange={onSetDatasource} value={datasourceName}
	placeholder='Select datasource...'>
                                {Object.keys(datasources).map((datasourceName) =>
                                    <option key={datasourceName} value={datasourceName}>{datasourceName}</option>)}
                            </Select>
                        </FormControl>
                        ...
                        <FormControl id='bucket' className={classes.control}>
                            ...
                            <Select onChange={onSetBucket} value={bucketSize} isDisabled={!isBucketizationEnabled}
	placeholder='Select bucket size...'>
                                {hasValue(datasourceName) ? datasources[datasourceName].variants.map((bucket) =>
                                    <option key={bucket.bucketSize}
	value={bucket.bucketSize}>{bucket.bucketSize}</option>) : null}
                            </Select>
                        </FormControl>
